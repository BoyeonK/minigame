...
저장소 전역 규칙(인코딩, 프로토콜 코드 생성, 네이밍)은 루트 [`../CLAUDE.md`](../CLAUDE.md)를 먼저 볼 것.

**`.cs` 파일은 CP949다.** Edit/Write 도구로 수정하면 한글 주석이 통째로 소실된다. 반드시 `sed -i`를 쓸 것 — 자세한 절차와 검증 방법은 루트 CLAUDE.md의 "인코딩" 항목에 있다.
