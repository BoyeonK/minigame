...
- [x] (2026-09-09 #10) `objectPool`·`RaceRoom` 수정분을 x64로 재빌드하고 Race 2인 매칭을 실제로 돌려 검증했다. 러너 오브젝트 id가 플레이어 인덱스대로 배정되어 클라이언트의 `S_R_ResponseState.playerid` 대조가 맞아떨어지는 것을 확인했고, 풀 해제 순서 변경 이후에도 매치 진행 중 크래시가 재현되지 않았다.
...
- [x] (2026-09-09 #11) README `2. 데모`를 실제 자료로 채웠다. 게임이 돌아가는 상태가 된 뒤 스크린샷 4장(최초 화면·로비·패스 러너·호박 쪼개기)을 찍어 `docs/images/`에 넣고 `Screenshot / GIF` 자리표시자를 대체했으며, 플레이 영상 자리에는 YouTube 링크를 걸었다. 데모 자료가 채워지기를 기다리던 문장·구성 정리도 이번에 마무리해 현 상태로 확정했다.
