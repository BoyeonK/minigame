...
public:
	...
	void DoAsync(Ret(T::*memFunc)(Args...), Args&&... args) {
		...
		Push({ objectPool<Job>::alloc(ownerWRef, memFunc, forward<Args>(args)...), objectPool<Job>::dealloc });
	}

	template<typename T, typename Ret, typename... Args>
	void DoAsyncAfter(Ret(T::* memFunc)(Args...), Args&&... args) {
		weak_ptr<T> ownerWRef = static_pointer_cast<T>(shared_from_this());
		Push({ objectPool<Job>::alloc(ownerWRef, memFunc, forward<Args>(args)...), objectPool<Job>::dealloc }, true);
	}
	...
	void DoTimerAsync(uint64_t tickAfter, Ret(T::* memFunc)(Args...), Args&&... args) {
		weak_ptr<T> ownerWRef = static_pointer_cast<T>(shared_from_this());
		shared_ptr<Job> job = { objectPool<Job>::alloc(ownerWRef, memFunc, forward<Args>(args)...), objectPool<Job>::dealloc };
		GJobTimer->Reserve(tickAfter, shared_from_this(), job);
	}
